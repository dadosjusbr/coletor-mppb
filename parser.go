...
func Parse(arquivos []string, chave_coleta string) (*coleta.FolhaDePagamento, error) {
	...
	if err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("erro tentando recuperar os dados de indenizações: %q", err))
	}
	...
	for _, f := range arquivos {
		...
		if err != nil {
			return nil, status.NewError(status.InvalidInput, fmt.Errorf("erro na tentativa de transformar os dados em matriz (%s): %q", f, err))
		}
		if len(dados) == 0 {
	...
	if parseErr {
		return &coleta.FolhaDePagamento{ContraCheque: folha}, status.NewError(status.Unknown, fmt.Errorf("parse error"))
	}
...
func criaMembro(membro []string, indenizacoes []string, chaveColeta string, counter int, fileName string) (*coleta.ContraCheque, error) {
	...
	if err != nil {
		return nil, status.NewError(status.InvalidInput, fmt.Errorf("error na transformação das remunerações: %q", err))
	}
...
func processaRemuneracao(membro []string, indenizacoes []string) ([]*coleta.Remuneracao, error) {
	...
	if err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("erro processando verbas indenizatorias 1: %q", err))
	}
	...
	if err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("erro processando outras remuneracoes temporarias 2: %q", err))
	}
	remuneracoes = append(remuneracoes, temp...)

	temp, err = criaRemuneracao(membro, coleta.Remuneracao_R, REMUNERACAO_BASICA, coleta.Remuneracao_B)
	if err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("erro processando remuneracao básica: %q", err))
	}
	remuneracoes = append(remuneracoes, temp...)

	temp, err = criaRemuneracao(membro, coleta.Remuneracao_R, REMUNERACAO_EVENTUAL_TEMPORARIA, coleta.Remuneracao_O)
	if err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("erro processando remuneracao eventual temporaria: %q", err))
	}
	remuneracoes = append(remuneracoes, temp...)

	temp, err = criaRemuneracao(membro, coleta.Remuneracao_D, OBRIGATORIOS_LEGAIS, coleta.Remuneracao_O)
	if err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("erro processando erro processando obrigatório/legais: %q", err))
	}
	remuneracoes = append(remuneracoes, temp...)
...
func criaRemuneracao(planilha []string, natureza coleta.Remuneracao_Natureza, categoria string, tipoReceita coleta.Remuneracao_TipoReceita) ([]*coleta.Remuneracao, error) {
	...
	for key := range headersMap[categoria] {
		...
		if err != nil {
			return nil, status.NewError(status.InvalidFile, fmt.Errorf("error buscando o valor na planilha: %q", err))
		}
...
func dadosParaMatriz(file string) ([][]string, error) {
	...
	f, err := ods.Open(file)
	if err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("ods.Open error(%s): %q", file, err))
	}
	defer f.Close()
	f.ParseContent(&doc)
	fileType := tipoCSV(file)
	if err := assertHeaders(doc, fileType); err != nil {
		return nil, status.NewError(status.InvalidFile, fmt.Errorf("assertHeaders() for %s error: %q", file, err))
	}
